...
@interface UpGodAttriViewController ()
...
@implementation UpGodAttriViewController
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)showGodAttriView:(NSDictionary *)data
{
    ...
    if ([data objectForKey:@"EquipId"])
    {
        ...
        self.attriLevelLabel.text = [[[equip godAttri] attriLevel] stringValue];
        self.attriValueLabel.text = [[[equip godAttri] attriValue] stringValue];
        
        [[self attributeIcon] setAttriID:[[[equip godAttri] attriId] intValue]];
        ...
        float fCurExp = [[[equip godAttri] attriExp] floatValue];
        float fMaxExp = [[equip getGodAttributeMaxExp] floatValue];
...
-(void)onClickedOnQQEquipDefaultView:(NSDictionary*)params
{
    ...
    [tempDict setObject:INT_TO_NUMBER(1) forKey:@"God"];
