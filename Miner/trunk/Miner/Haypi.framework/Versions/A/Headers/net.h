...
typedef void* HaypiJson;
typedef void* HaypiSocket;
...
int base64_encode_(void * dst, const void * src, uint32_t len);
int base64_decode_(void * dst, const void * src, uint32_t len);
uint32_t getCrc32Code(unsigned char* pData, int length);
...
uint16_t getCLastUserGCPort(int gameMode);
